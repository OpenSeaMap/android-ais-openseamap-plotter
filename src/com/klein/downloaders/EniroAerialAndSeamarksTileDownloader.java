...
import com.klein.commons.Device;
...
public class EniroAerialAndSeamarksTileDownloader extends TileDownloader {
	    ...
	    private String stdDir =  "/"+ AISPlotterGlobals.DEFAULT_APP_DATA_DIRECTORY; // "/AISPlotter"
	    //private String mCacheDirPathStr =  stdDir + mCachePathStr;
		private String mCacheDirPathStr = "/"+ AISPlotterGlobals.DEFAULT_ONLINE_TILE_CACHE_DATA_DIRECTORY +"/EniroAerialWithSeamarks";
		 ...
		 */
		public  EniroAerialAndSeamarksTileDownloader(Context context,int aTimeout) {
			super();
			mContext = context;
		...
		/*public void XsetTileCacheSubDirName (String aName){
			if (aName.startsWith("/")){
		...
		private boolean getTileFromCache(String cacheFilePathStr, Bitmap bitmap){
		    ...
		    if (fileExist) {
	            ...
	            if (test) Log.d(TAG,"found in Cache: " + cacheFilePathStr);
		...
		public boolean executeJob(MapGeneratorJob mapGeneratorJob, Bitmap bitmap) {
			...
			try {
		        ...
		        con.setReadTimeout(mTimeoutForDownload  * 2);
		        con.connect(); 
