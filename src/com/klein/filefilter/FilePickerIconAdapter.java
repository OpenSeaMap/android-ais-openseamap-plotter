	...
	public View getView(int index, View convertView, ViewGroup parent) {
		...
		if (index == 0 && this.hasParentFolder) {
			...
			if (this.currentFile.isDirectory()) {
				...
				String info = this.currentFile.getName();
				long aFileSize = this.currentFile.length();
				if (aFileSize < 1000) {
				  info = info + " "  + this.currentFile.length() + " byte";
				} else if (aFileSize < 1000000) {
					info = info + " "  + this.currentFile.length()/1000 + " Kb";
				} else {
					info = info + " "  + this.currentFile.length()/1000000 + " Mb";
				}
